...
contract TestBetting {
    ...
    function testClaimingSuspendedBet() public { // test claiming rewards if the match is suspended
        payable(betting).transfer(marketInitialBalance);
        require(better1.makeBet(50 wei, 1), "Better must be able to bet A");
        require(better2.makeBet(60 wei, 1), "Better must be able to bet B");
        oracle.setWinner(1);
        Assert.isFalse(better1.claimWinningBets(), "Better should not be able to claim winning bets if betting on match is suspended. C");
    }
    ...
    function testClaimingLosingBets() public { // test claiming losing bets by bet market maker
        ...
        Assert.isTrue(address(bettingMaker).balance == marketInitialBalance * 2 + 90 wei, "Bet market maker should be able to claim losing bets");
    }
