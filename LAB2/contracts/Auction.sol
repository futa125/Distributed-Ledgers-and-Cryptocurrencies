...
contract Auction {
    ...
    modifier onlyJudgeOrHighestBider() {
        if (judgeAddress != address(0)) {
            require(
                msg.sender == judgeAddress || msg.sender == highestBidderAddress
            );
        }
        _;
    }

    modifier onlyJudgeOrSeller() {
        if (judgeAddress != address(0)) {
    ...
    modifier onlyWhenOutcome(Outcome _outcome) {
        require(outcome == _outcome);
        _;
    }
    ...
    function finalize()
        public
        onlyJudgeOrHighestBider
        onlyWhenOutcome(Outcome.SUCCESSFUL)
    {
    ...
    function refund()
        public
        onlyJudgeOrSeller
        onlyWhenOutcome(Outcome.NOT_SUCCESSFUL)
    {
        require(highestBidderAddress != address(0));
        (bool success, ) = highestBidderAddress.call{
